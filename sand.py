...
from boundary_conditions import energy_input, test, energy_input_data, sample_holder_data, calculate_L_chamber_lamp_bd, calculate_deeper_layer_source, day_night_cycle, calculate_L_chamber_lamp_from_image, get_sample_holder_temp_array
...
--> Cool down:      9865-15192 its
--> Day-night:      15193-20736 its
'''
...
for albedo in albedo_arr:
    for lambda_sand_c in lambda_arr:
        for abs_depth in absorption_depth_arr:
            epsilon, epsilon_ambient = 0.95, 0.95
            start_time = np.datetime64('2023-12-27 19:34:43')
            ...
            for type in [1]:
                ...
                boundary_temp = get_sample_holder_temp_array('C:/Users/Christian Schuckart/OneDrive/Work/Paper - sand/SultansOfSand_Boundary.txt', start_time, const.dt, const.k)
                '''
                ...
                for j in tqdm(range(0, const.k)):
                    ...
                    temperature = sample_holder_data(const.n_x, const.n_y, const.n_z, sample_holder, temperature, boundary_temp[j])
                    if j < 9865:
                ...
                print(name_string)
                data_dict = {'Right': sensors_right.tolist(), 'Rear': sensors_rear.tolist()}
                with open(name_string, 'w') as outfile:
