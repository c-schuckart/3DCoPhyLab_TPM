...
ax5 = plt.subplot2grid(shape=(3, 5), loc=(1, 2), colspan=1, rowspan=1)
ax6 = plt.subplot2grid(shape=(3, 5), loc=(1, 3), colspan=1, rowspan=1)
ax7 = plt.subplot2grid(shape=(3, 5), loc=(1, 4), colspan=1, rowspan=1)
ax8 = plt.subplot2grid(shape=(3, 5), loc=(2, 0), colspan=1, rowspan=1)
...
temps_mean = []
temps_median = []
current_file = 'D:/TPM_Data/Ice/second_best_fit_std_lamp_temps_high_res.csv'
with open(current_file) as csvdatei:
    dat = csv.reader(csvdatei)
    i = 0
    for each in dat:
        temps_max.append(float(each[0]))
        temps_mean.append(float(each[1]))
        temps_median.append(float(each[2]))

time = [i * const.dt for i in range(const.k)]

ax.plot(time, temps_max, label='Max. Temp.')
ax.plot(time, temps_mean, label='Mittelwert Temp.', ls='--')
ax.plot(time, temps_median, label='Median Temp.', ls='-.')
ax.set_xlabel('Zeit (s)')
ax.set_ylabel('Temperatur (K)')
#plt.savefig('C:/Users/Christian/OneDrive/Uni/Master/3 - Masterarbeit/Ice/Thesis/second_best_fit_lamp_temps.png', dpi=600)
plt.legend()
plt.show()'''
...
for i in range(0, 1):
    ...
    json_file.close()
    n_z, n_y, n_x = np.shape(temp)
    for a in range(n_z):
        if temp[a][n_y//2][n_x//2] > 0:
...
axes[0].tick_params(axis='both', which='both', direction='in', top=True, right=True, bottom=True, left=True, labeltop=False, labelright=False, labelbottom=True, labelleft=True)
axes[1].tick_params(axis='both', which='both', direction='in', top=True, right=True, bottom=True, left=True, labeltop=False, labelright=False, labelbottom=True, labelleft=True)
axes[0].set_prop_cycle(color=[cm(1. * i / NUM_COLORS) for i in range(NUM_COLORS)])
...
for label in labels:
    if label[0:5] == 'Right' and count == plot_only[ind]-1:
        axes[0].plot(time[0:19230], ((data[label] + shift).apply(f) + 273.15)[45770:65000], label=ger_labels[count], color=colors[ind])
        ...
        for positions in label_pos_2:
            if abs(cur_pos - positions) < 1.5:
                cur_pos -= np.sign(- cur_pos + positions) * 2.5
        axes[0].text(195000, cur_pos, ger_labels[count], fontsize='xx-small')
        label_pos_2.append(cur_pos)
    elif label[0:4] == 'Rear' and count == plot_only[ind]-1:
        axes[0].plot(time[0:19230], ((data[label] + shift).apply(f) + 273.15)[45770:65000], label=ger_labels[count], color=colors[ind])
