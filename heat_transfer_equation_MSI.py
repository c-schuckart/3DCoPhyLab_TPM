...
def solve_iterative(iterations, temperature, q, A, L, U):
    T = temperature.flatten()
    R = q - np.dot(A, T)
    for i in range(iterations):
        V = solve_triangular(L, R, trans=True, unit_diagonal=False)
        delta = solve_triangular(U, V, trans=False, unit_diagonal=True)
        T = T + delta
        R = q - np.dot(A, T)
    return T + delta
