...
@njit
def calculate_molecule_surface(n_x, n_y, n_z, temperature, pressure, a_1, b_1, c_1, d_1, m_H2O, dx, dy, dz, dt, surface_reduced, k_B, water_mass_per_layer):
    ...
    return sublimated_mass, empty_voxels[0:empty_voxel_count], pressure, outgassed_mass/dt
...
@njit
def sinter_neck_calculation_time_dependent(r_n, r_p, dt, temperature, a_1, b_1, c_1, d_1, omega, surface_energy, R_gas, r_grain, alpha, m_mol, density, pressure, m_H2O, k_B, k_factor, water_particle_number, blocked_voxels, n_x, n_y, n_z, sample_holder, dx, dy, dz, surface, sinter_time_reducer):
    p_sub = np.zeros((n_z, n_y, n_x), dtype=np.float64)
    sublimated_mass = np.zeros((n_z, n_y, n_x), dtype=np.float64)
    for i in range(1, n_z-1):
        for j in range(1, n_y-1):
            for k in range(1, n_x-1):
                if temperature[i][j][k] > 0 and sample_holder[i][j][k] == 0:
                    if blocked_voxels[i][j][k] == -1:
                        sublimated_mass[i][j][k] = 10 ** (a_1[0] + b_1[0] / temperature[i][j][k] + c_1[0] * np.log10(temperature[i][j][k]) + d_1[0] * temperature[i][j][k]) * np.sqrt(m_H2O/(2 * np.pi * k_B * temperature[i][j][k])) * dt * (dx[i][j][k] * dy[i][j][k] * (surface[i][j][k][1] + surface[i][j][k][0]) + dx[i][j][k] * dz[i][j][k] * (surface[i][j][k][2] + surface[i][j][k][3]) + dy[i][j][k] * dz[i][j][k] * (surface[i][j][k][4] + surface[i][j][k][5]))
                        p_sub[i][j][k] = 0
                        pressure[i][j][k] = 0
                    elif blocked_voxels[i][j][k] == 0:
                        p_sub[i][j][k] = 10 ** (a_1[0] + b_1[0] / (temperature[i][j][k] - sinter_time_reducer) + c_1[0] * np.log10((temperature[i][j][k] - sinter_time_reducer)) + d_1[0] * (temperature[i][j][k] - sinter_time_reducer))
                    ...
                    Z_sinterneck = (p_sub[i][j][k] - pressure[i][j][k]) * np.sqrt(m_H2O/(2 * np.pi * k_B * (temperature[i][j][k] - sinter_time_reducer)))
                    r_c = 2 * m_mol * surface_energy / (density[i][j][k] * R_gas * temperature[i][j][k])
                    r_p[i][j][k] = r_p[i][j][k] - Z_sinterneck * np.exp(r_c/r_p[i][j][k]) * dt / density[i][j][k]
                    ...
                    rate = ((omega**2 * surface_energy * p_sub[i][j][k])/(R_gas * temperature[i][j][k] - sinter_time_reducer) * 1/np.sqrt(2*np.pi * m_mol * R_gas * temperature[i][j][k] - sinter_time_reducer) * d_s / (d_s + delta * np.arctan(r_p[i][j][k]/(r_n[i][j][k] + delta))) * (2/r_p[i][j][k] + 1/delta - 1/r_n[i][j][k]) - Z_sinterneck/density[i][j][k] * np.exp(- r_c/(r_n[i][j][k] * k_factor)))
...
@njit
def sinter_neck_calculation_time_dependent_diffusion_t(r_n, r_p, dt, temperature, a_1, b_1, c_1, d_1, omega, surface_energy, R_gas, r_grain, alpha, m_mol, density, pressure, m_H2O, k_B, k_factor, water_particle_number, blocked_voxels, n_x, n_y, n_z, sample_holder, dx, dy, dz, gas_density, areas, surface, sinter_reduction_factor):
    p_sub = np.zeros((n_z, n_y, n_x), dtype=np.float64)
    sublimated_mass = np.zeros((n_z, n_y, n_x), dtype=np.float64)
    #areas = np.zeros((n_z, n_y, n_x), dtype=np.float64)
    for i in range(1, n_z-1):
        for j in range(1, n_y-1):
            for k in range(1, n_x-1):
                if temperature[i][j][k] > 0 and sample_holder[i][j][k] == 0:
                    ...
                    Z = (p_sub[i][j][k] - pressure[i][j][k]) * np.sqrt(m_H2O/(2 * np.pi * k_B * temperature[i][j][k]))
                    r_c = 2 * m_mol * surface_energy / (density[i][j][k] * R_gas * temperature[i][j][k])
                    r_p[i][j][k] = r_p[i][j][k] - Z * np.exp(r_c/r_p[i][j][k]) * dt / density[i][j][k] * sinter_reduction_factor
                    #r_p = r_grain - (r_grain / (r_grain - r_c)) * Z * time*dt / density
                    delta = r_n[i][j][k]**2 / (2 * (r_p[i][j][k] - r_n[i][j][k])) * k_factor
                    d_s = r_p[i][j][k] * (alpha/2 + np.arctan(r_p[i][j][k]/(r_n[i][j][k] + delta)) - np.pi/2)
                    rate = ((omega**2 * surface_energy * p_sub[i][j][k])/(R_gas * temperature[i][j][k]) * 1/np.sqrt(2*np.pi * m_mol * R_gas * temperature[i][j][k]) * d_s / (d_s + delta * np.arctan(r_p[i][j][k]/(r_n[i][j][k] + delta))) * (2/r_p[i][j][k] + 1/delta - 1/r_n[i][j][k]) - Z/density[i][j][k] * np.exp(- r_c/(r_n[i][j][k] * k_factor)))
                    ...
                    #cond_rate = (omega**2 * surface_energy * p_sub[i][j][k])/(R_gas * temperature[i][j][k]) * 1/np.sqrt(2*np.pi * m_mol * R_gas * temperature[i][j][k]) * d_s / (d_s + delta * np.arctan(r_p[i][j][k]/(r_n[i][j][k] + delta))) * (2/r_p[i][j][k] + 1/delta - 1/r_n[i][j][k]) * neck_area
                    #sublimated_mass = sublimated_mass + (Z * (water_particle_number * np.exp(r_c/r_p) * 4 * np.pi * r_p**2 + 3 * np.exp(-r_c/(r_n * k_factor)) * neck_area) - 3 * cond_rate) * dt
                    if blocked_voxels[i][j][k] == 1 and np.sum(surface[i][j][k]) != 0:
                        sublimated_mass[i][j][k] = 10 ** (a_1[0] + b_1[0] / temperature[i][j][k] + c_1[0] * np.log10(temperature[i][j][k]) + d_1[0] * temperature[i][j][k]) * np.sqrt(m_H2O/(2 * np.pi * k_B * temperature[i][j][k])) * dt * (dx[i][j][k] * dy[i][j][k] * (surface[i][j][k][1] + surface[i][j][k][0]) + dx[i][j][k] * dz[i][j][k] * (surface[i][j][k][2] + surface[i][j][k][3]) + dy[i][j][k] * dz[i][j][k] * (surface[i][j][k][4] + surface[i][j][k][5]))
                    if blocked_voxels[i][j][k] == 1 and delta > 0 and np.sum(surface[i][j][k]) == 0:
                        areas[i][j][k] = (water_particle_number[i][j][k] * 4 * np.pi * r_p[i][j][k] ** 2 + 3 * neck_area)
                        sublimated_mass[i][j][k] = Z * (water_particle_number[i][j][k] * np.exp(r_c / r_p[i][j][k]) * 4 * np.pi * r_p[i][j][k] ** 2 + 3 * np.exp(-r_c / (r_n[i][j][k] * k_factor)) * neck_area) * dt
                        v = np.sqrt(8 * k_B * temperature[i][j][k] / (np.pi * m_H2O))
                        pressure_impulse = 1 / areas[i][j][k] * sublimated_mass[i][j][k] * v ** 2 / (2 * r_p[i][j][k])  # 2 * r_mono ~~ mean free path
                    ...
                    #if np.isnan(sublimated_mass[i][j][k]):
                        #print(Z[i][j][k], water_particle_number[i][j][k], r_p[i][j][k], neck_area[i][j][k], cond_rate[i][j][k], delta[i][j][k], r_n[i][j][k])
                    r_n[i][j][k] = r_n[i][j][k] + dt * rate * sinter_reduction_factor
    return r_n, r_p, sublimated_mass, areas
'''@njit
def sinter_neck_calculation_time_dependent_diffusion(r_n, r_p, dt, temperature, a_1, b_1, c_1, d_1, omega, surface_energy, R_gas, r_grain, alpha, m_mol, density, pressure, m_H2O, k_B, k_factor, water_particle_number, blocked_voxels, n_x, n_y, n_z, sample_holder, dx, dy, dz, gas_density, areas):
    p_sub = np.zeros((n_z, n_y, n_x), dtype=np.float64)
        ...
        for j in range(1, n_y-1):
            for k in range(1, n_x-1):
                if temperature[i][j][k] > 0 and sample_holder[i][j][k] == 0:
                    ...
                    r_c = 2 * m_mol * surface_energy / (density[i][j][k] * R_gas * temperature[i][j][k])
                    areas[i][j][k] = (water_particle_number[i][j][k] * 4 * np.pi * r_p[i][j][k] ** 2)
                    sublimated_mass[i][j][k] = Z * (water_particle_number[i][j][k] * np.exp(r_c / r_p[i][j][k]) * 4 * np.pi * r_p[i][j][k] ** 2) * dt
                    v = np.sqrt(8 * k_B * temperature[i][j][k] / (np.pi * m_H2O))
                    pressure_impulse = 1 / areas[i][j][k] * sublimated_mass[i][j][k] * v ** 2 / (2 * r_p[i][j][k])  # 2 * r_mono ~~ mean free path
                    #pressure_impulse = 1/6 * 1 / (dx[i][j][k] * dy[i][j][k]) * sublimated_mass[i][j][k] * v**2 / (2 * r_p[i][j][k])    # 2 * r_mono ~~ mean free path
                    if pressure_impulse > p_sub[i][j][k]:
        ...
        for b in range(0, n_y):
            for c in range(0, n_x):
                if temperature[a][b][c] > 0 and sample_holder[a][b][c] != 1 and areas[a][b][c] > 0:
                    ...
                    pressure[a][b][c] = 1/areas[a][b][c] * (gas_density[a][b][c] * dx[a][b][c] * dy[a][b][c] * dz[a][b][c]) * v**2 / (2 * r_mono[a][b][c])    # 2 * r_mono ~~ mean free path
...
@njit
def Tsinter_neck_calculation_time_dependent_diffusion(r_n, r_p, dt, temperature, a_1, b_1, c_1, d_1, r_mono, pressure, m_H2O, k_B, k_factor, water_particle_number, blocked_voxels, n_x, n_y, n_z, sample_holder, dx, dy, dz, gas_density, areas):
    p_sub = np.zeros((n_z, n_y, n_x), dtype=np.float64)
    sublimated_mass = np.zeros((n_z, n_y, n_x), dtype=np.float64)
    #areas = np.zeros((n_z, n_y, n_x), dtype=np.float64)
    for i in range(1, n_z-1):
        for j in range(1, n_y-1):
            for k in range(1, n_x-1):
                if temperature[i][j][k] > 0 and sample_holder[i][j][k] == 0:
                    p_sub[i][j][k] = 10 ** (a_1[0] + b_1[0] / temperature[i][j][k] + c_1[0] * np.log10(temperature[i][j][k]) + d_1[0] * temperature[i][j][k])
                    sublimated_mass[i][j][k] = (10 ** (a_1[0] + b_1[0] / temperature[i][j][k] + c_1[0] * np.log10(temperature[i][j][k]) + d_1[0] * temperature[i][j][k]) - pressure[i][j][k]) * np.sqrt(m_H2O/(2 * np.pi * k_B * temperature[i][j][k])) * areas[i][j][k] * dt
                    v = np.sqrt(8 * k_B * temperature[i][j][k] / (np.pi * m_H2O))
                    pressure_impulse = 1/areas[i][j][k] * sublimated_mass[i][j][k] * v**2 / (2 * r_mono[i][j][k])    # 2 * r_mono ~~ mean free path
                    if pressure_impulse > p_sub[i][j][k]:
                        sublimated_mass[i][j][k] = p_sub[i][j][k] * areas[i][j][k] * 2 * r_mono[i][j][k] / (v**2) - (gas_density[i][j][k] * dx[i][j][k] * dy[i][j][k] * dz[i][j][k])
                    #areas[i][j][k] = dx[i][j][k] * dy[i][j][k]
